...
public class GuiAchievementsNext extends GuiAchievements {
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_) {
        if (this.progress) {
            ...
            this.drawCenteredString(this.fontRendererObj, field_146510_b_[(int) (Minecraft.getSystemTime() / 150L % (long) field_146510_b_.length)], this.width / 2, this.height / 2 + this.fontRendererObj.FONT_HEIGHT * 2,
                    16777215);
        } else {
            int k;
    ...
    protected void func_146552_b(int p_146552_1_, int p_146552_2_, float p_146552_3_) {
        ...
        for (i3 = 0; (float) i3 * f1 - (float) l2 < 155.0F; ++i3) {
            ...
            for (j3 = 0; (float) j3 * f2 - (float) k2 < 224.0F; ++j3) {
                ...
                if (ir < 0) ir *= -1;
                if (ir == 0) ir = 1;
                k3 = random.nextInt(ir) + (j2 + i3) / 2;
                IIcon iicon = this.getIcon(random, k3, j2, i3);
    ...
    private void setBrightness(int j2, int i3) {
        ...
        if (currentPage != -1 && AchievementPage.getAchievementPage(currentPage) instanceof IAchievementIcon) {

            float f3 = ((IAchievementIcon) AchievementPage.getAchievementPage(currentPage)).getBrightness(j2, i3);
    ...
    private IIcon getIcon(Random random, int k3, int j2, int i3) {

        IIcon iicon = Blocks.sand.getIcon(0, 0);

        if (currentPage != -1 && AchievementPage.getAchievementPage(currentPage) instanceof IAchievementIcon) return ((IAchievementIcon) AchievementPage.getAchievementPage(currentPage)).getIcon(random, k3, j2, i3);
