...
public class TileEntitySuctionMachine extends TileEntityDirection implements IFluidHandler, IGearForceHandler, IGearForceGrid {
	...
	public TileEntitySuctionMachine() {
	...
	public void updateServerEntity()
	{
		...
		if (cooltime > 10) {
			...
			if (storage.drawEnergy(2, 40, false) >= 34) {
				this.updateServerOutWorkEntity();
				this.updateServerInWorkEntity();
				this.getFluidFromSuctionMachine();
			}
	...
	public void updateServerInWorkEntity()
	{
		...
		if (f != null && this.tank.getFluid() == null) {
			...
			if (this.tank.getFluid() == null) {
				...
				if (t.getTankInfo(direction.getOpposite()) != null && t.getTankInfo(direction.getOpposite())[0].fluid != null && t.canDrain(this.direction.getOpposite(), t.getTankInfo(direction.getOpposite())[0].fluid.getFluid())) {
	...
	public void updateServerOutWorkEntity()
	{
		...
		if (f != null) {
			...
			} else if (this.worldObj.getTileEntity(x, y, z) instanceof IFluidHandler && ((IFluidHandler) this.worldObj.getTileEntity(x, y, z)).canFill(ForgeDirection.DOWN, f.getFluid())) {
	...
	private void getFluidFromSuctionMachine() {

		if (!(this.getWorldObj().getTileEntity(xCoord, yCoord - 1, zCoord) instanceof TileEntityPipe)) return;

		TileEntityPipe p = (TileEntityPipe) this.getWorldObj().getTileEntity(xCoord, yCoord - 1, zCoord);

		p.getFluidFromSuctionMachine(ForgeDirection.UP);
